...
func solution(_ S : String) -> String {
    let stringWithoutSpaces = S.replacingOccurrences(of: " ", with: "")
    let stringWithoutSpacesAndDashes = stringWithoutSpaces.replacingOccurrences(of: "-", with: "")
    
    var result = ""
    var count = -2
    for c in stringWithoutSpacesAndDashes {
        result.append(c)
        if count % 3 == 0 {
            result.append("-")
        }
